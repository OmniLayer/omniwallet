    ...
    var self = this;
      ...
      self.getUnsignedTransaction = function(type, data){
        if (type == 0 && data.currency_identifier == 0){ // BTC send
          data = {
            'from_address':data.pubkey, 'to_address':data.transaction_to, 'amount':data.amount_to_transfer.times(SATOSHI_UNIT).valueOf(), 'currency':'BTC', 'fee':data.fee,'marker': (data.marker || false)
          };
