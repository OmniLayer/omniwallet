  ...
  CryptUtilAsync = (function() {
    ...
    return {
    	generateNonceForDifficulty: function( challengeString, onComplete ) {
    	  ...
    	  cryptoWorker.postMessage({ name: 'generateNonceForDifficulty', challengeString: challengeString });
	    },
	    ...
	    generateSymmetricKey: function( password, hexSalt, onComplete ) {
    	  submittedJobs.push({ onComplete : onComplete })
    	  cryptoWorker.postMessage({ name: 'generateSymmetricKey', password: password, hexSalt: hexSalt });
	    },

	    encryptObject: function( o, key, onComplete ) {
    	  submittedJobs.push({ onComplete : onComplete })
    	  cryptoWorker.postMessage({ name: 'encryptObject', o: o, key: key });
	    },

	    decryptObject: function( string, key, onComplete ) {
    	  submittedJobs.push({ onComplete : onComplete })
    	  cryptoWorker.postMessage({ name: 'decryptObject', string: string, key: key });
	    },

	    createSignedObject: function( data, privKey, onComplete ) {
    	  submittedJobs.push({ onComplete : onComplete })
    	  cryptoWorker.postMessage({ name: 'createSignedObject', data: data, privKey: privKey });
	    }
    };
  })();
} else {
